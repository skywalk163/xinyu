...
from src.error_handling import (
    ErrorHandler, ErrorType, ErrorCode, ErrorSeverity,
    create_error_message, get_error_info
)
from src.error_handling.enhanced_errors import EnhancedErrorMessages
from src.error_handling.error_utils import create_syntax_error, create_lexical_error
...
def test_error_utils_functions():
    """测试错误工具函数"""
    # 测试创建语法错误
    syntax_error_msg = create_syntax_error(
        token=";",
        line=8,
        column=3,
        source="定 x 为 42;"
    ...
    # 测试创建词法错误
    lexical_error_msg = create_lexical_error(
        char="@",
        line=12,
        column=7,
        source="打印(@hello)"
...
def test_error_suggestions():
    ...
    # 测试缩进错误建议
    indent_suggestions = suggest_fix(
