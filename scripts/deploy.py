...
class DeploymentSystem:
    ...
    def run_command(
        self, cmd: str, cwd: Optional[Path] = None, capture_output: bool = True, check: bool = True
    ) -> Tuple[bool, str]:
        """运行命令"""
        if cwd is None:
        ...
        try:
            result = subprocess.run(
                cmd, shell=True, cwd=cwd, capture_output=capture_output, text=True, check=check
            )
    ...
    def run_code_quality_checks(self) -> bool:
        ...
        checks = [
            ...
            (
                "代码风格 (flake8)",
                "flake8 src/ tests/ tools/ --max-line-length=100 --ignore=E203,W503,E501",
            ),
            ("心语代码格式化", "python tools/xinyu_format.py check src/ tests/ examples/"),
            ("类型检查 (mypy)", "mypy src/ --ignore-missing-imports"),
            (
                "安全扫描 (bandit)",
                "bandit -r src/ -ll --skip B101,B105,B106,B107,B108,B110,B112,B113,B404,B603,B607",
            ),
...
def main():
    ...
    parser.add_argument(
        "--environment", "-e", choices=["test", "production"], default="test", help="部署环境"
    )
