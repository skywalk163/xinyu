...
from src.security import PermissionManager, PermissionType, SecureContext, AuditLogger, AuditedPermissionManager
...
def test_audited_permission_manager():
    ...
    try:
        ...
        for record in records:
            print(f"  - {record.user}: {record.operation} {record.resource} -> {'允许' if record.allowed else '拒绝'}")
...
def test_integration():
    ...
    try:
        ...
        records = audit_logger.get_audit_trail()
        for record in records:
            print(f"  [{record.timestamp}] {record.user}({record.role}): {record.operation} {record.resource} -> {'允许' if record.allowed else '拒绝'}")
