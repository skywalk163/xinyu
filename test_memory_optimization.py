...
from src.memory import (
    ObjectPool, LexerPool, ParserPool, TokenFlyweight,
    MemoryMonitor, get_memory_monitor,
    MemoryOptimizer, get_memory_optimizer,
    TokenFlyweightStrategy, ASTNodePoolStrategy, CacheMemoryLimitStrategy, StringInterningStrategy,
    estimate_memory_usage, track_memory_allocation, find_memory_leaks,
    optimize_memory_usage, monitor_memory_growth, generate_memory_report,
...
def test_memory_profiling():
    ...
    if profile_result['allocation_hotspots']:
        ...
        for i, hotspot in enumerate(profile_result['allocation_hotspots'][:3], 1):
            print(f"    热点 {i}: {hotspot['filename']}:{hotspot['lineno']} - {hotspot['size_diff_kb']:.2f}KB")
    ...
    # 监控内存增长
    monitor_result = monitor_memory_growth(
        interval=0.2,
        duration=1.0,
        threshold_mb=2.0
    )
...
def test_integration():
    ...
    for i, obj_info in enumerate(report['object_analysis'][:5], 1):
        print(f"  {i}. {obj_info['type_name']}: {obj_info['count']} 个, {obj_info['total_size']/1024:.1f}KB")
