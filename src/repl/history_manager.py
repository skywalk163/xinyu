...
class HistoryManager:
    ...
    def _detect_command_type(self, command: str) -> CommandType:
        ...
        # 检查是否为表达式（以值结尾）
        if (command.startswith('计算') or 
            command.startswith('求值') or
        ...
        # 检查是否为控制语句
        if (command.startswith('如果') or command.startswith('否则') or
            command.startswith('循环') or command.startswith('当') or
            command.startswith('对于') or command.startswith('重复')):
            return CommandType.CONTROL
        
        # 检查是否为调试命令
        if (command.startswith('调试') or command.startswith('断点') or
            command.startswith('查看') or command.startswith('跟踪')):
    ...
    def _edit_in_database(self, index: int, new_command: str) -> Optional[HistoryEntry]:
        ...
        # 获取原始条目
        self.cursor.execute(
            "SELECT * FROM history ORDER BY timestamp DESC LIMIT 1 OFFSET ?",
            (index,)
        )
    ...
    def export_csv(self, filepath: Path) -> None:
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            ...
            # 写入表头
            writer.writerow([
                'timestamp', 'command', 'result', 'command_type',
                'execution_time', 'success', 'tags', 'metadata'
            ])
    ...
    def _get_stats_from_database(self) -> Dict[str, Any]:
        ...
        # 平均执行时间
        self.cursor.execute("SELECT AVG(execution_time) FROM history WHERE execution_time IS NOT NULL")
...
# 便捷函数
def create_history_manager(max_size: int = 1000, 
                          use_database: bool = False) -> HistoryManager:
    """创建历史记录管理器
