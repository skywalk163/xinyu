...
from .history_manager import HistoryManager, HistoryEntry, CommandType
...
class CodeCompletionProvider:
    ...
    def __init__(self, compiler=None):
        ...
        self.keywords = [
            "如果", "否则", "循环", "当", "为", "在", "定义", "返回",
            "导入", "从", "作为", "类", "尝试", "捕获", "最终",
            "引发", "断言", "通过", "继续", "中断", "打印", "输入"
        ]
        self.builtin_functions = [
            "打印", "输入", "长度", "类型", "整数", "浮点数", "字符串",
            "列表", "字典", "范围", "枚举", "排序", "反转", "求和",
            "最大值", "最小值", "绝对值", "四舍五入", "向上取整", "向下取整"
        ]
    ...
    def complete(self, text: str, pos: int) -> List[str]:
        ...
        # 获取当前单词
        line = text[:pos]
        word_start = max(line.rfind(' '), line.rfind('\t'), line.rfind('\n'), line.rfind('('), 
        ...
        # 内置函数补全
        suggestions.extend([func for func in self.builtin_functions if func.startswith(current_word)])
...
class SyntaxHighlighter:
    ...
    def _highlight_line(self, line: str) -> str:
        ...
        if '#' in line:
            ...
            highlighted = self._highlight_code(code_part) + self.comment_color + comment_part + self.reset_color
            return highlighted
