...
# 使用统一的导入工具
from src.utils.imports import import_optional, with_psutil, with_tracemalloc, with_gc, with_time
...
def estimate_memory_usage(obj: Any, seen: Optional[set] = None) -> int:
    ...
    if isinstance(obj, dict):
        size += sum(estimate_memory_usage(k, seen) + estimate_memory_usage(v, seen) 
...
def find_memory_leaks(snapshot_interval: float = 1.0, 
                     duration: float = 10.0,
...
def optimize_memory_usage(target_mb: Optional[float] = None,
    ...
    if aggressive:
        ...
        optimizations.append({
            'name': '清除循环引用',
            'description': f'清除了 {cleared_cycles} 个循环引用',
            'memory_saved_mb': 0.0
        })
        ...
        try:
            ...
            if hasattr(module, '__dict__'):
                ...
                for attr_name in module.__dict__.keys():
                    # 查找缓存属性
                    if (attr_name.startswith('_cache_') or 
                        attr_name.endswith('_cache') or
                        attr_name == '__cache__' or
                        attr_name.startswith('cache_')):
...
def monitor_memory_growth(interval: float = 1.0, 
                         duration: float = 30.0,
...
def generate_recommendations(memory_usage: MemoryUsage,
                           object_analysis: List[ObjectInfo],
