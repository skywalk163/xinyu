...
class TokenFlyweightStrategy(MemoryOptimizationStrategy):
    ...
    def __init__(self):
        super().__init__(name="token_flyweight", description="使用享元模式共享Token实例，减少重复Token对象的内存占用")
        self._pool = {}
        self._hits = 0
        self._misses = 0
...
class ASTNodePoolStrategy(MemoryOptimizationStrategy):
    ...
    def __init__(self, max_pool_size: int = 1000):
        super().__init__(name="ast_node_pool", description="使用对象池复用AST节点，减少节点创建和垃圾回收开销")
        ...
        try:
            ...
            return OptimizationResult(
                ...
                details={
                    ...
                    "reuses": self._reuses,
                    "reuse_rate": self._reuses / (self._allocations + self._reuses)
                    if (self._allocations + self._reuses) > 0
                    else 0,
...
class CacheMemoryLimitStrategy(MemoryOptimizationStrategy):
    ...
    def __init__(self, max_cache_memory_mb: float = 100.0):
        super().__init__(name="cache_memory_limit", description="为编译缓存添加内存使用限制，自动淘汰旧条目")
        ...
        try:
            ...
            class MemoryLimitedCache(CompilationCache):
                ...
                def _evict_if_needed(self):
                    ...
                    # 淘汰条目直到内存使用低于限制
                    while (
                        entries and self.current_memory_bytes > self.max_memory_bytes * 0.8
                    ):  # 保留20%缓冲
                        key, _ = entries.pop(0)
                    ...
                    stats.update(
                        {
                            ...
                            "memory_usage_percent": (
                                self.current_memory_bytes / self.max_memory_bytes * 100
                            )
...
class StringInterningStrategy(MemoryOptimizationStrategy):
    ...
    def __init__(self):
        super().__init__(name="string_interning", description="使用字符串驻留技术，减少重复字符串的内存占用")
