...
# 使用统一的导入工具
from src.utils.imports import import_optional, with_psutil, with_tracemalloc, with_gc, with_time, with_json, with_os
...
class MemoryMonitor:
    ...
    def _check_for_leaks(self) -> Optional[MemoryLeakReport]:
        ...
        # 计算内存增长
        memory_growth = recent_snapshots[-1].current_memory_mb - recent_snapshots[0].current_memory_mb
    ...
    def compare_snapshots(self, 
        ...
        return {
            ...
            'memory_growth_percent': (
                ...
                snapshot1.current_memory_mb * 100
                if snapshot1.current_memory_mb > 0 else 0
            )
        }
    ...
    def generate_report(self, 
        ...
        with self._lock:
            ...
            report = {
                ...
                'recent_leak_reports': [
                    asdict(report) for report in self._leak_reports[-5:]
                ] if self._leak_reports else [],
...
def get_memory_monitor(check_interval: float = 5.0,
                      memory_threshold_mb: float = 100.0) -> MemoryMonitor:
    """获取全局内存监控器实例"""
...
def start_memory_monitoring(check_interval: float = 5.0,
                           memory_threshold_mb: float = 100.0) -> MemoryMonitor:
    """启动内存监控"""
...
def check_for_memory_leaks(snapshot_interval: float = 10.0,
                          duration: float = 60.0) -> Optional[MemoryLeakReport]:
    """
    检查内存泄漏
