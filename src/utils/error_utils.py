...
class ErrorRegistry:
    ...
    def create_error(
        self, code: str, context: Optional[ErrorContext] = None, **format_args
    ) -> BaseError:
        """
        创建错误对象
        ...
        if not error_info:
            return BaseError(
                message=f"未知错误代码: {code}", code=code, severity=ErrorSeverity.ERROR, context=context
            )
...
def create_error(code: str, context: Optional[ErrorContext] = None, **format_args) -> BaseError:
    """
    创建错误对象（快捷函数）
...
def _log_exception(exception: Exception, function_name: str, args: tuple, kwargs: dict) -> None:
    """记录异常"""
    import logging
