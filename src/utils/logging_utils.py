        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if elapsed > threshold:
                    logger.warning(
                        f"函数 {module_name}.{func_name} 执行缓慢: {elapsed:.4f}秒 "
                        f"(阈值: {threshold}秒)"
                    )
                else:
                    logger.log(
                        level,
                        f"函数 {module_name}.{func_name} 执行时间: {elapsed:.4f}秒"
                    )
                ...
                elapsed = time.time() - start_time
                logger.error(
                    f"函数 {module_name}.{func_name} 执行失败，耗时: {elapsed:.4f}秒，异常: {e}"
                )
                raise
