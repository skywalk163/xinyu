...
def track_memory_allocation(func: Callable = None, *, 
                           track_calls: bool = True,
                           track_lines: bool = False) -> Callable:
    """
    跟踪函数内存分配的装饰器
...
def find_memory_leaks(snapshot1: Any = None, 
                     snapshot2: Any = None,
...
def compare_snapshots(snapshot1: MemorySnapshot, 
...
class MemoryProfiler:
    ...
    def stop(self) -> Dict[str, Any]:
        ...
        for i in range(len(self.snapshots) - 1):
            ...
            stage_diffs.append({
                'from': self.snapshots[i].label,
                'to': self.snapshots[i + 1].label,
                'diff': diff
            })
