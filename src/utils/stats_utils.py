...
def calculate_percentiles(data: List[float], 
                         percentiles: List[float] = [25, 50, 75, 90, 95, 99]) -> Dict[str, float]:
    """
    计算百分位数
    ...
    for p in percentiles:
        ...
        if pos.is_integer():
            ...
            result[f'p{p}'] = (sorted_data[idx_low] * weight_low + 
                              sorted_data[idx_high] * weight_high)
...
def format_statistics(stats: Dict[str, Any], 
                     precision: int = 4,
                     include_percentiles: bool = True) -> str:
    """
    格式化统计信息为字符串
...
def calculate_frequency_distribution(data: List[Any], 
