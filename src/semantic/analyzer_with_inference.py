...
class SemanticAnalyzerWithInference:
    ...
    def _report_error(
        ...
        self.error_handler.report(
            ErrorType.SEMANTIC_ERROR, message, line, column, suggestion=suggestion
        )
    ...
    def _visit_var_def(self, node: VarDefNode) -> str:
        ...
        self.current_scope.define(
            node.name, "variable", value_type=inferred_type, line=node.line, column=node.column
        )
    ...
    def _visit_for(self, node: ForNode) -> str:
        ...
        # 定义循环变量
        self.current_scope.define(
            node.var, "variable", value_type="unknown", line=node.line, column=node.column
        )
    ...
    def _visit_function_def(self, node: FunctionDefNode) -> str:
        ...
        if self.current_scope.has(node.name):
            self._report_error(
                f"函数 '{node.name}' 已定义", node.line, node.column, suggestion="请使用不同的函数名"
            )
            return "unknown"
