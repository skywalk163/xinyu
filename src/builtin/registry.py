...
class BuiltinRegistry:
    ...
    def register_all_builtins(self) -> None:
        """注册所有69个内置函数"""
        # 导入所有内置函数实现
        from .functions import (  # 数学函数; 类型转换函数; 序列操作函数; 对象操作函数; IO函数; 其他函数
            builtin_abs,
            builtin_all,
            builtin_any,
            builtin_ascii,
        ...
        self.register(
            BuiltinFunction("divmod", "除法余数", builtin_divmod, (2, 2), "返回除法的商和余数", ["除余"])
        )
        ...
        self.register(
            BuiltinFunction("frozenset", "转冻结集合", builtin_frozenset, (1, 1), "转换为冻结集合", ["冻结集合"])
        )
        self.register(BuiltinFunction("bytes", "转字节", builtin_bytes, (1, 2), "转换为字节", ["字节"]))
        self.register(
            BuiltinFunction("bytearray", "转字节数组", builtin_bytearray, (1, 2), "转换为字节数组", ["字节数组"])
        )
        self.register(
            BuiltinFunction("memoryview", "转内存视图", builtin_memoryview, (1, 1), "创建内存视图", ["内存视图"])
        )
        ...
        self.register(
            BuiltinFunction("isinstance", "是实例", builtin_isinstance, (2, 2), "实例检查", ["实例检查"])
        )
        self.register(
            BuiltinFunction("issubclass", "是子类", builtin_issubclass, (2, 2), "子类检查", ["子类检查"])
        )
        self.register(
            BuiltinFunction("hasattr", "有属性", builtin_hasattr, (2, 2), "属性存在检查", ["属性存在"])
        )
        ...
        self.register(
            BuiltinFunction("ascii", "ASCII表示", builtin_ascii, (1, 1), "ASCII表示", ["ASCII"])
        )
        ...
        self.register(
            BuiltinFunction("callable", "可调用", builtin_callable, (1, 1), "是否可调用", ["是否可调用"])
        )
