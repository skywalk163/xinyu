...
class BenchmarkReporter:
    ...
    def generate_text_report(self, results: List[BenchmarkResult], title: str = "基准测试报告") -> str:
        """
        生成文本格式报告
        ...
        sorted_results = sorted(
            results, key=lambda r: r.statistics.get("time_mean", 0), reverse=True
        )
        ...
        if len(results) > 1:
            ...
            report_lines.append(
                f"最快测试: {fastest.name} ({fastest.statistics.get('time_mean', 0):.6f}s)"
            )
            report_lines.append(
                f"最慢测试: {slowest.name} ({slowest.statistics.get('time_mean', 0):.6f}s)"
            )
    ...
    def generate_html_report(
        self, results: List[BenchmarkResult], title: str = "基准测试报告", include_charts: bool = True
    ) -> str:
        """
        生成HTML格式报告
...
"""
    ...
    def generate_json_report(
        self, results: List[BenchmarkResult], title: str = "基准测试报告"
    ) -> Dict[str, Any]:
        """
        生成JSON格式报告
        ...
        if results:
            report_data["summary"] = {
                ...
                "average_time": sum(r.statistics.get("time_mean", 0) for r in results)
                / len(results),
                "average_memory": sum(r.statistics.get("memory_mean", 0) for r in results)
                / len(results),
    ...
    def save_report(
        self, results: List[BenchmarkResult], title: str = "基准测试报告", formats: List[str] = None
    ) -> Dict[str, str]:
        """
        保存报告到文件
