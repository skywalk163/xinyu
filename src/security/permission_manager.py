...
class PermissionManager:
    ...
    def _create_default_policy(self) -> None:
        ...
        developer_role = Role(
            ...
            permissions=[
                ...
                Permission(
                    "*.py", [PermissionType.READ, PermissionType.EXECUTE], "开发者可以执行Python文件"
                ),
        ...
        user_role = Role(
            ...
            permissions=[
                Permission(
                    "/home/user/*", [PermissionType.READ, PermissionType.WRITE], "用户可以读写自己的home目录"
                ),
    ...
    def _save_policy(self) -> None:
        """保存安全策略"""
        try:
            policy_data = {"roles": {}, "role_hierarchy": self.role_hierarchy}

            for role_name, role in self.roles.items():
                role_data = {"permissions": [], "parent_roles": role.parent_roles}
    ...
    def _match_permission(
        self, permission: Permission, operation: PermissionType, resource: str
    ) -> bool:
        """检查权限是否匹配"""
        # 检查操作匹配
        if (
            PermissionType.ALL not in permission.operations
            and operation not in permission.operations
        ):
            return False
    ...
    def validate_resource_access(
        self, role: str, operation: PermissionType, resource: str
    ) -> Dict[str, Any]:
        """验证资源访问权限，返回详细结果"""
