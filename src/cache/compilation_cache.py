...
class CompilationCache:
    ...
    def set_tokens(self, source: str, tokens: List) -> None:
        ...
        self._token_cache[hash_key] = CacheEntry(
            value=tokens,
            timestamp=time.time(),
            size=size
        )
    ...
    def set_ast(self, source: str, ast: Any) -> None:
        ...
        self._ast_cache[hash_key] = CacheEntry(
            value=ast,
            timestamp=time.time()
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'token_hit_rate': self._stats['token_hits'] / total_token_requests if total_token_requests > 0 else 0,
            'ast_hit_rate': self._stats['ast_hits'] / total_ast_requests if total_ast_requests > 0 else 0,
            **self._stats
        }
