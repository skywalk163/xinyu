...
class MacroExpander:
    ...
    def expand_ast(self, node: ASTNode) -> ASTNode:
        ...
        try:
            ...
            if isinstance(node, ProgramNode):
                ...
                return ProgramNode(
                    line=node.line, column=node.column, statements=expanded_statements
                )
                ...
                if self.macro_system.has(node.name):
                    ...
                    return FunctionCallNode(
                        name=node.name, args=expanded_args, line=node.line, column=node.column
                    )
                ...
                expanded_else = (
                    [self.expand_ast(stmt) for stmt in node.else_branch]
                    if node.else_branch
                    else None
                )
                return IfNode(
    ...
    def _expand_for_loop(self, node: ForNode) -> ASTNode:
        ...
        macro_call = FunctionCallNode(
            name="遍历",
            args=[
                IdentifierNode(name=node.var, line=node.line, column=node.column),
                node.iterable,
                node.body,
            ],
            line=node.line,
    ...
    def _expand_repeat_loop(self, node: RepeatNode) -> ASTNode:
        ...
        # 将重复循环转换为宏调用
        macro_call = FunctionCallNode(
            name="重复", args=[node.count, node.body], line=node.line, column=node.column
        )
        expanded = self._expand_macro_call(macro_call)
        # 直接返回展开结果，不包装成列表
        return expanded
