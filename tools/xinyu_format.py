...
try:
    from tools.formatter import FormatterConfig, XinyuFormatter
except ImportError:
    # 如果导入失败，尝试直接导入
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    from formatter import FormatterConfig, XinyuFormatter
...
def main():
    ...
    changed_files, total_issues = format_files(
        xinyu_files, formatter, in_place=args.in_place, check_only=args.check, verbose=args.verbose
    )
