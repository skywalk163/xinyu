...
class SimpleXinyuFormatter:
    ...
    def format_code(self, source: str) -> str:
        ...
        for line in lines:
            ...
            if not stripped:
                ...
                if (
                    not formatted_lines
                    or formatted_lines[-1].strip() != ""
                    or self.count_trailing_empty_lines(formatted_lines)
                    < self.config.get("max_empty_lines", 2)
                ):
    ...
    def _adjust_indent_level(self, current_level: int, line: str) -> int:
        ...
        # 增加缩进的情况
        if (
            stripped.startswith("定义")
            or stripped.startswith("如果")
            or stripped.startswith("否则")
            or stripped.startswith("对于")
            or stripped.startswith("当")
            or stripped.startswith("重复")
            or stripped.endswith(":")
        ):
            return current_level + 1

        # 减少缩进的情况
        if stripped.startswith("返回") or stripped.startswith("结束") or stripped == "否则:":
